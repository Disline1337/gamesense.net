...
void CInterfaces::Initialize()
{
	...
	GameMovement = ClientManager->BruteInterface<IGameMovement>(xorstr_("GameMovement"));
	...
	ClientState = **(CClientState***)((*reinterpret_cast<uintptr_t**>(Engine))[14] + 0x1);
	Prediction = ClientManager->BruteInterface<IPrediction>(xorstr_("VClientPrediction"));
	...
	MoveHelper = **reinterpret_cast<IMoveHelper***>(Cheat::Utilities->Memory_PatternScan(xorstr_("client.dll"), xorstr_("8B 0D ? ? ? ? 8B 45 ? 51 8B D4 89 02 8B 01")) + 2);
